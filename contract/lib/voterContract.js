...
class MyAssetContract extends Contract {
  ...
  async init(ctx) {
    ...
    let voter2 = await new Voter('V2', '345', 'Ahmed', 'Ali');
...
*/
    ...
    //create votableItems for the ballots
    let repVotable = await new VotableItem(ctx, 'Shaheen', ballotData.fedDemocratBrief);
    let demVotable = await new VotableItem(ctx, 'Merit', ballotData.republicanBrief);
    let indVotable = await new VotableItem(ctx, 'Mashaal', ballotData.greenBrief);
    let grnVotable = await new VotableItem(ctx, 'Flambueue', ballotData.independentBrief);
    let libVotable = await new VotableItem(ctx, 'United', ballotData.libertarianBrief);
  ...
  async createVoter(ctx, args) {
    ...
    //create a new voter
    let newVoter = await new Voter(args.cms,"voterId123", args.firstName, args.lastName, args.fingerPrint);

    //update state with new voter
    await ctx.stub.putState(newVoter.cms, Buffer.from(JSON.stringify(newVoter)));
    ...
    let response = `voter with voterId ${newVoter.voterId}, with first name ${newVoter.firstName}, with last name ${newVoter.lastName} is now updated in the world state`;
    return response;
  }

async createPanel(ctx,args){
args = JSON.parse(args);
//crate new panel
let newPanel = await new VotableItem(args.panelId, args.panelName);

//update the state with new panel
await ctx.stub.putState(newPanel.panelId,Buffer.from(JSON.stringify(newPanel)));


  let response = `Panel with panelId $(newPanel.panelId) and panel name $(newPanel.panelName)`;
    return response;
